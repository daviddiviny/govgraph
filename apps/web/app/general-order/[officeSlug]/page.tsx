    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 py-8 sm:px-8 lg:px-10 lg:py-12">
      <header className="grid gap-6 rounded-[2rem] border border-[var(--gg-color-border)] bg-white/75 p-6 shadow-[0_24px_80px_rgba(14,44,36,0.08)] backdrop-blur-sm lg:grid-cols-[1.2fr_0.8fr] lg:p-8">
        <div className="space-y-5">
          <div className="flex flex-wrap gap-4 text-sm font-medium uppercase tracking-[0.2em] text-[var(--gg-color-semantic-text-secondary)]">
        ...
        {state.status === "ready" ? (
          <Card className="p-5">
            ...
            <h2 className="mt-4 text-xl font-semibold text-[var(--gg-color-ink)]">
            ...
            <a
              ...
              className="mt-4 inline-flex text-sm font-medium text-[var(--gg-color-accent)] underline-offset-4 hover:underline"
            >
              Open official page
            </a>
          </Card>
      ...
      {state.status === "empty" ? (
        <EmptyState
          ...
          description="This office page needs the imported General Order rows in Postgres before it can show anything."
        />
      ) : null}
      ...
      {state.status === "error" ? (
        <EmptyState
          ...
          description={`The app could not read the imported General Order data for this office. Error: ${state.message}`}
        />
      ) : null}
                ...
                <div className="mt-5 grid gap-4">
                  {act.rules.map((rule, index) => (
                    <div
                      ...
                      className="rounded-[1.5rem] border border-[var(--gg-color-border)] bg-[var(--gg-color-paper)]/70 p-4"
                    >
                      <div className="flex flex-wrap gap-3">
                      ...
                      <p className="mt-4 text-base leading-7 text-[var(--gg-color-ink)]">
                        {rule.rawText}
                      </p>
                      {rule.administeringOfficeNames.length > 1 ? (
                        <p className="mt-3 text-sm leading-6 text-[var(--gg-color-semantic-text-secondary)]">
